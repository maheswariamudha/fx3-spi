        ...
        uint8_t input)
{
	...
	uint16_t xsub_pos;
		...
		//xsub_pos = (input/17) * 26;
	if(input <= 255 && input > 0)
	{
	xsub_pos = input * 1.53;
	xsub_pos = 390 - xsub_pos;
	...
	//{
		...
		exp_buf[0]=0x00;
		exp_buf[1]=0x00;
		exp_buf[2]=(uint8_t)((xsub_pos&0xff00)>>8);
		exp_buf[3]=(uint8_t)((xsub_pos&0x00ff));
		SensorWrite(SENSOR_ADDR_WR,0x14,0x00,4,exp_buf); //LED
	}
	//}
