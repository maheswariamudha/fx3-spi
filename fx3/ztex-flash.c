...
uint8_t ztex_flash_wait() {
	...
	ztex_flash.ec = ztex_flash.cs ? FLASH_EC_PENDING :
					ztex_flash.enabled ? 0 : FLASH_EC_NOTSUPPORTED;
	if (ztex_flash.ec)
	...
	errreturn: ztex_flash.ec = FLASH_EC_RUNTIME;
	ztex_flash_deselect();
	return ztex_flash.ec;
}
...
uint8_t ztex_flash_read(uint8_t* buf, uint32_t addr, uint32_t size) {
	...
	while (size > 0) {
		s2 = size;
		if (s2 > ZTEX_SPI_PAGE_SIZE)
			s2 = ZTEX_SPI_PAGE_SIZE;
		...
		ZTEX_REC(
				status = CyU3PDmaChannelSetupRecvBuffer(&ztex_spi_rd_handle,
						&buf_p));
		if (status != CY_U3P_SUCCESS)
			goto errreturn;

		ZTEX_REC(
				status = CyU3PDmaChannelWaitForCompletion(&ztex_spi_rd_handle,
						500));
		if (status != CY_U3P_SUCCESS) {
...
uint8_t ztex_flash_pp() {
	...
	ZTEX_LOG("ztex_flash.buf[0] = %x", ztex_flash.buf[0]);
	ZTEX_LOG("ztex_flash.buf[0] = %x", ztex_flash.buf[1]);
	ZTEX_LOG("ztex_flash.buf[0] = %x", ztex_flash.buf[2]);
	ZTEX_LOG("ztex_flash.buf[0] = %x", ztex_flash.buf[3]);
...
uint8_t ztex_flash_write(uint8_t* buf, uint32_t size) {
	...
	if (ztex_flash.need_pp && ztex_flash_pp())
		return ztex_flash.ec;
	...
	while (size > 0) {
		...
		ZTEX_REC(
				status = CyU3PDmaChannelSetupSendBuffer(&ztex_spi_wr_handle,
						&buf_p));

		ZTEX_REC(
				status = CyU3PDmaChannelWaitForCompletion(&ztex_spi_wr_handle,
						500));
		if (status != CY_U3P_SUCCESS) {
		...
		if (size == 0) {
			...
			if (ztex_flash_pp())
				return ztex_flash.ec; // finish pp + wait
		}
	}
...
uint8_t vr_flash_info(uint16_t value, uint16_t index, uint16_t length) {
	if (ztex_flash.ec == 0 && ztex_flash.cs)
		ztex_flash.ec = FLASH_EC_PENDING;
...
uint8_t vr_flash_read(uint16_t value, uint16_t index, uint16_t length) {
	...
	if (ztex_flash_read(ztex_ep0buf, ztex_flash.ep0_read_addr, length))
		return 255;
...
uint8_t vc_flash_write(uint16_t value, uint16_t index, uint16_t length) {
	...
	if (index == 0 && ztex_flash_write_start(value))
		return 255;

	ZTEX_REC_RET(CyU3PUsbGetEP0Data(length, ztex_ep0buf, NULL));
	if (ztex_flash_write(ztex_ep0buf, length))
		return 255;

	if (index == 3)
		ztex_flash_write_finish();
	return 0;
}
...
//    spiConfig.clock      = 10000000;
	...
	ZTEX_REC(
			status = CyU3PDmaChannelCreate(&ztex_spi_wr_handle,
	...
	ZTEX_REC(
			status = CyU3PDmaChannelCreate(&ztex_spi_rd_handle,
	...
	if (status != CY_U3P_SUCCESS)
		goto errreturn;
	ztex_flash.device =
			CyU3PSpiReceiveWords(ztex_flash.buf, 2) == CY_U3P_SUCCESS ?
					ztex_flash.buf[1] : 127;
	ztex_flash_deselect();
